...
- `manna migrate` now repairs the Stage 0 legacy-board lock without weakening strict mode. One HMAC-authenticated, board-locked transaction generates sealed handoffs for active items, grandfathers done history, exempts tracks and dreams, releases unauthenticated legacy claims, updates Git visibility, and publishes strict identity last. Recovery is exact-state and idempotent; ordinary strict commands cannot invoke migration or use it to reseal edited work.
