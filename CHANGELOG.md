...
- `manna serve` now joins coord: NOW rows carry the claimant's pulse (needs-you / failed / working, current tool, latest prompt, todo progress) and sort attention-first; a per-board COORDINATION section shows who is here, what they hold, advisory claims with contention derived from overlapping live owners, and drops; the estate index at `/` carries needs-you / working / here per board. DRIFT is live (`manna reconcile --json`, read-only, on board change) with the written file's age beside it. Board and git state re-derive on file signature; presence re-reads on a ten-second cadence and pushes only on a content change, because presence files churn on every tool call. (mn-3dd0f8, mn-3a58e4, mn-2eeaa6)
- `agent-do manna serve` is the human window onto the board: one read-only daemon on `127.0.0.1:7777` renders every registered board at `/` and each project at `/<name>` with NOW (claimed work and the claimant's liveness from coord), NEXT (open, unblocked, in `handoff-order.yaml` priority), NEEDS DECISION (leading `[DECISION]`/`[HUMAN]`/`[OWNER]` title tags, plus any a machine adds with `--decision-marker`), WAITING (the `blocked_by` graph in topological waves), DRIFT (the last reconcile's `drift.yaml`, never re-run by the page), by-track groups, dreams, and a grep/filter table; item drawers show every commit carrying the item's `Manna:` trailer and copy the handoff path, the id, or the `show` command in one click. Live over server-sent events on board, git, and coord presence changes. Running `serve` inside a project registers its board, starts the daemon if needed, and always prints the project URL, so an agent asked for the board hands over the link; `--scan <dir>` registers every board below a directory, `--status`/`--stop` manage the daemon. Agents keep `manna context|list|show`; the page is a rendering, never a source, and private claim proofs never leave the board directory. (mn-613088; generalized from codex's Moon Two control room.)
- Legacy Manna boards are now discoverable before the first failed write. A nonempty identityless board directs callers to `manna migrate` rather than the non-convergent `init` loop; `bootstrap --recommend`, ordinary bootstrap, and SessionStart classify the same state and surface the one-command migration path. `manna lint` also reports every durable board file absent from the Git index as `workflow_tracking` with `git-tracked: no`, distinguishing visible files from history-protected state.
- Cursor SessionStart now persists the conversation id for machine-key-derived Manna ownership instead of minting a process-mortal random token. Cursor lanes therefore recover the same claim authority after a restart, stale half-pinned identities are neutralized, and complete explicit identity pairs still take precedence.
- `manna init` is now an HMAC-authenticated, board-locked, crash-recoverable transaction. It preserves existing issue and session bytes, prepares Git-visible workflow/order/index state, and publishes `board.yaml` only after every prerequisite exists. A killed initializer leaves authenticated recovery intent rather than a permanently identityless board; the next init completes the exact transaction, repeated init is byte-stable, and the recovery directory is empty at rest.
- Ordered Manna handoffs no longer fail when the active plan exceeds 99 items. Priority and blocker fields use one board-wide width with a two-digit minimum, expanding to `001..N` and `b001..N` at 100 items so lexical filename order stays truthful. Crossing a width boundary rides the existing authenticated rename transaction; claimed files remain immovable and hold their numbers until release.
- `manna migrate` now converges the remaining estate-sweep source shapes. A frontmatter-like legacy document without a row binding is imported as content even when it contains its own old Claim section; strict validation scopes authority to the canonical Claim section instead of rejecting preserved prose. Description-first pointers may carry a historical `PROMPT: path.md — note` annotation; the note is context rather than filename, and rerunning migration repairs rows admitted by the preceding parser through a content-preserving authenticated rebind. Active absolute pointers inside the project normalize to repository-relative provenance, while absolute cross-project Markdown pointers are copied into new canonical sealed handoffs with the original path recorded. Imported in-project sources outside `.handoff/` move into deterministic `.handoff/.archive/legacy-sources/*.source` evidence after their exact bytes are sealed into every consuming handoff, removing the shadow workflow without deleting its content or mutating an external project. The authenticated journal carries source bytes, archive paths, and target-before bytes, so recovery proves preservation and non-overwrite; rerunning `migrate` also repairs boards admitted before source retirement shipped without changing unaffected strict rows or seals. Existing row bindings remain strict authority: deleting a real strict handoff binding still fails closed and cannot be laundered through migration.
- Manna handoffs now render the board as a launch plan. `.manna/handoff-order.yaml` owns first-class priority; `manna order` and `manna sync` derive dense `<NN>[b<MM>]-mn-<id>-<slug>.md` names plus a generated README index. The single `bMM` marker names the highest-numbered still-open blocker, while complete dependencies remain `blocked_by` edges. A native HMAC-authenticated `Rename` transaction stages cycles safely, repoints exact board rows, and commits priority and index bytes together. Live claimed files never move and keep their number reserved until release; completed pairs return to unnumbered history so every bare numbered name remains a truthful launch signal. Lint and reconcile enforce any remaining presentation drift. Owner-authenticated abandon now releases blocked claims without discarding their blocker state.
- `manna migrate` now repairs the Stage 0 legacy-board lock without weakening strict mode, including boards where a v2 create landed before migration. One HMAC-authenticated, board-locked transaction passes valid strict pairs through byte-for-byte, adopts malformed legacy `.handoff/` files without losing their work-order contents, generates missing sealed pairs, grandfathers done history, exempts tracks and dreams, releases unauthenticated legacy claims, seeds first-class priority from unique handmade prefixes, updates Git visibility, and publishes strict identity last. Recovery is exact-state and idempotent; ordinary strict commands cannot invoke migration or use it to reseal edited work.
- Manna now enforces one transactional workflow spine: `.manna/board.yaml` pins strict or legacy identity independently of the repairable version-2 workflow config, and tracked `.handoff/` is the sole work-order root. Lifecycle mutations re-read and write under one board lock, claimed rows enforce exact session ownership, and `update --status` cannot bypass lifecycle verbs. Strict handoffs use symlink-safe project paths, structured metadata, and a SHA-256 content binding mirrored in the issue; `manna handoff seal` authorizes intentional edits. A write-ahead journal recovers paired row/file creates, edits, conversions, and deletes, while `lint`, `reconcile`, and `claim` fail closed on ignored board state, broken pairs, or workflow sprawl anywhere in the repository. `bootstrap` restores deleted strict config without silently downgrading the board and classifies genuine pre-workflow boards once as legacy.
- Manna integrity is fail-closed under hostile local state and concurrency: pair journals are HMAC-authenticated outside the worktree, atomically installed without replacement, semantically restricted to canonical `.handoff/` paths, and replayed only against exact complete rows. Reconcile fixes compare the inspected row before releasing a dead claim. Only explicit `handoff seal` may create a new content binding; metadata changes, config restoration, version downgrade, and `done` cannot bless or hide edits. Storage rejects intermediate symlinks and ancestor Git ignores, shadow detection covers generic claim-bearing Markdown plus neutral internal and external symlink roots, and claim ownership requires a private proof in addition to the public session ID. Claude and Cursor persist stable host session ids, while Codex supplies its opaque thread id; Manna derives each proof under a machine-local key, and plain shells fail closed without an explicit pair.
...
- The launcher, installer, health checker, and test runner now enforce GNU Bash 4.4+ through one Bash 3.2-safe selector. Sparse macOS agent environments promote to a verified Homebrew Bash through an isolated Bash-only PATH shim, preserving caller-provided Python and CLI fixtures. Health fails closed when credential probing crashes or returns malformed output, and the Render logs suite covers the empty-filter path that exposed the runtime mismatch.
