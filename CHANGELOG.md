...
### Added
- `agent-do context` freshness-aware `retrieve`, conditional refresh, active source registration/sync, bounded maintenance, and trust/provenance controls for current agent reference docs.
- `agent-do gh` for GitHub repository, pull request, review, and merge work-state across accessible repos, with `inbox`, `awaiting`, `prs`, `pr`, `diff`, `threads`, `checks`, `review`, `audit`, `approve`, `request-changes`, `comment`, `close`, `reopen`, `checkout`, `edit`, `update-branch`, `merge`, `ready`, and `draft` commands.
...
- Hook outcome telemetry under `~/.agent-do/telemetry/events.jsonl`, correlating hook emissions with subsequent `agent-do` tool calls/results and classifying nudges as followed, ignored, expired, or pending.
