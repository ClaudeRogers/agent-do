...
- `bootstrap` now installs one workflow spine in every detected project: `.manna/` is the only backlog and tracked `.handoff/` is the only work-order root. New Manna boards record workflow version 1, generate a bidirectionally linked handoff for each item, reject shadow prompt roots, and fail closed before claim when the file, pointer, Git visibility, or exact claim command drifts. `lint` and `reconcile` expose linkage and `workflow_sprawl` findings, while existing nonempty boards remain legacy until deliberately migrated.
