...
- `manna serve` is now a cockpit, ratified through four wireframe rounds: three tabs (inbox · board · coordination) with a breadcrumb to the estate, grep, and a ⌘K jump palette; an inspector on the right; a status strip carrying live drift and daemon health that opens a debug sheet. The inbox lists every ask in one shape (who/what · the ask · the verb you perform). Rows show a one-line digest generated by the fast model (hash-keyed cache outside the board, byte-bounded batches against the model's input window, a measured per-call cap, transient failures retried after a cooldown, title as fallback); the manna title lives in the inspector. Finish: 12px floor, one-line zebra rows, severity stripes, outlined pills, prompt headers; dpt floors clean (100% contrast pass, nothing under 12px). (mn-1e71e2)
