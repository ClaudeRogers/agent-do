...
| `tools/agent-coord` | Python | Project-local state-and-interrupt broker (v2). Identities are session UUIDs anchored to the long-lived agent process (pid + start time), so a recycled tmux pane never inherits a dead session; presence is liveness-verified (kill -0 + start-time match) into active/idle/dead/stopped/stale with last-seen ages. Stores state under `.git/agent-do/coord/`, tracks structured focus (goal/phase/note/blocking_on/last_ship), roles with exclusive-writer territories, claims/needs/publishes/drops, and computes contention/notice/dependency/novelty interrupts instead of relying on a mailbox. `stop`/`bye` retire sessions cleanly; `guard install` adds a warn-only pre-commit overlap check; `history` reads the events journal. |
