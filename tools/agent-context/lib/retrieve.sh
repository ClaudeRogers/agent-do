...
cmd_retrieve() {
    ...
    local fresh=false require_fresh=false offline=false require_official=false
    ...
    if [[ -z "$query" ]]; then
        if [[ "${OUTPUT_FORMAT:-text}" == "json" ]]; then
            json_error "Usage: agent-context retrieve <query> [--max-tokens N] [--fresh|--require-fresh] [--offline]"
            return 1
        fi
        die "Usage: agent-context retrieve <query> [--max-tokens N] [--fresh|--require-fresh] [--offline]"
    fi
...
PYTHON
    ...
    _context_retrieve_emit "$query" "$max_tokens" "$limit" "$fresh" "$require_fresh" "$offline" "$require_official" "$failures_file"
    local rc=$?
    rm -f "$failures_file"
    return "$rc"
}
...
_context_retrieve_emit() {
    local query="$1" max_tokens="$2" limit="$3" fresh="$4" require_fresh="$5" offline="$6" require_official="$7" failures_file="$8"
    python3 - "$CONTEXT_INDEX_DB" "$query" "$max_tokens" "$limit" "$fresh" "$require_fresh" "$offline" "$require_official" "$failures_file" "$CONTEXT_HOME/feedback.jsonl" "${OUTPUT_FORMAT:-text}" << 'PYTHON'
import json
import os
import sqlite3
import sys
...
if output_format == "json":
    ...
    print()
    print(f"Packages: {len(selected)} | Tokens: ~{result['used_tokens']}/{max_tokens} | Freshness: {'ok' if result['freshness_ok'] else 'stale'}")
    if failures:
    ...
    for item in selected:
        ...
        print(f"Trust: {item['trust']} | Freshness: {item['freshness']} | Checked: {item['checked_at'] or 'unknown'}")
        print()
        print(item["content"])
        print()
        print("---")
