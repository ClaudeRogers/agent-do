...
def _open_prs_for_author(author: str, limit: int) -> list[dict[str, Any]]:
    ...
    try:
        ...
        # Fall back: search without mergeStateStatus for older gh versions.
        try:
...
def _pr_repo_slug(pr: dict[str, Any]) -> str | None:
    # Prefer the pull-request URL, which points at the base repository.
    # For update-branch we need the destination repo, not the contributor fork.
    url = pr.get("url", "")
...
def _merge_base_into_branch(repo_dir: str, base_branch: str) -> tuple[bool, bool]:
    git = _git_bin()
    remotes = _run([git, "remote"], cwd=repo_dir, timeout=60).stdout.split()
    base_remote = "upstream" if "upstream" in remotes else "origin"
    _run([git, "fetch", base_remote], cwd=repo_dir, timeout=60)
    result = _run([git, "merge", f"{base_remote}/{base_branch}", "--no-edit"], cwd=repo_dir)
    if result.returncode == 0:
        return True, False
...
def cmd_sync(args: Any) -> None:
    ...
    for pr in prs:
        ...
        head_repo_slug = _pr_head_repo_slug(pr) or repo_slug
        ...
        if head_repo_slug and head_repo_slug != repo_slug:
            _log("    skipped: fork PR head differs from base repo")
            results.append({
                "pr": number,
                "title": title,
                "status": "skipped",
                "reason": "fork PR head differs from base repo",
            })
            continue
        ...
        with tempfile.TemporaryDirectory(prefix="agent-gh-sync-") as workdir:
            repo_dir = _clone_or_update(head_repo_slug or repo_slug, head_branch, workdir)
            if not repo_dir:
