...
def cmd_incidents(args: argparse.Namespace) -> int:
    ...
    if args.state:
        incidents = [
            ...
            if ((inc.get("attributes") or {}).get("state")
                or (inc.get("attributes") or {}).get("status")
                or "").lower() == args.state
        ]
    ...
    for inc in incidents:
        ...
        status = _text(attrs.get("state") or attrs.get("status"))
        severity = _text(attrs.get("severity")) or _incident_field_value(attrs, "severity")
