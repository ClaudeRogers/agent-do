...
Use `awaiting` for the queue, then add `--audit --replies` when the goal is review decision support instead of just discovery. The audit path inspects PR metadata, checks, unresolved threads, diff content, lockfile blast radius, deployment/env hints, and optional Render/Vercel env-key presence, then drafts engineering review text with concrete fix guidance.
