...
- Identity is host-anchored and authenticated: Claude/Cursor hooks pin `MANNA_SESSION_ID`, secret `MANNA_SESSION_TOKEN`, and `AGENT_DO_COORD_SESSION`; Codex derives the proof from its opaque thread id under a machine-local key. There is no transient-pid fallback, the visible owner label is not a credential, and scripted lanes export both Manna values explicitly.
