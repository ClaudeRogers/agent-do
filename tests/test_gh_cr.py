...
def base_env(fake_bin: Path, fake_home: Path) -> dict[str, str]:
    ...
    return {
        **os.environ,
        ...
        "AGENT_DO_LIVE_CONTEXT": json.dumps({
            "enabled": True,
            "scope": "any",
            "app": "GitHub",
            "ttl_seconds": 60,
            "reason": "gh:cr:address",
        }),
        "HOME": str(fake_home),
    }
...
def test_cr_list_shows_threads() -> None:
    """cr <pr> without --address prints unresolved review item table."""
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "unresolved review item" in result.stdout
        assert "src/main.py" in result.stdout
        assert "coderabbitai" in result.stdout


def test_cr_list_clean_pr() -> None:
    """cr <pr> on a PR with no unresolved review items reports clean."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        fake_bin = tmp / "bin"
        fake_bin.mkdir()
...
def test_cr_dry_run_no_address() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "2 review item" in result.stdout
...
def test_cr_address_requires_live() -> None:
    """cr <pr> --address requires +live before clone/claude/push."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        fake_bin = tmp / "bin"
        fake_bin.mkdir()
        ...
        make_fake_git(fake_bin, sha_counter_path=tmp / "sha-counter")
        make_fake_claude(fake_bin)
        env = base_env(fake_bin, fake_home)
        env.pop("AGENT_DO_LIVE_CONTEXT", None)
        env.pop("AGENT_DO_LIVE_SPEC", None)
        ...
        assert payload["action_required"] == "LIVE_APPROVAL_REQUIRED"
        assert payload["required_scope"] == "any"
        assert payload["app"] == "GitHub"
        assert payload["reason"] == "gh:cr:address"
        assert "+live(" in payload["rerun"]
...
""",
        ...
        assert result.returncode == 0, result.stderr
        assert "2 unresolved review item" in result.stdout
...
def test_cr_author_flag() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Custom fake gh that logs the list args
...
def test_cr_limit_flag() -> None:
    """cr --limit passes through to pr list."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        fake_bin = tmp / "bin"
        fake_bin.mkdir()
